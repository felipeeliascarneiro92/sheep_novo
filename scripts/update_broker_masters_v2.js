...
async function updateBrokerMasters() {
    console.log('🚀 Starting Broker Masters Update (Robust Version)...');
    ...
    // 3. Prepare Set of Master IDs (as Strings for safety)
    const masterLegacyIds = new Set(masters.map(m => m.id.toString()));
    ...
    for (const broker of dbBrokers) {
        if (!broker.legacy_id) continue;

        // Compare as strings
        if (masterLegacyIds.has(broker.legacy_id.toString())) {
            const currentPerms = broker.permissions || {};

            // If not already master permission
